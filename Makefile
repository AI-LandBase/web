...
init: ## 定番パッケージを含むNext.jsアプリケーションを作成（初回のみ実行）
	@if [ ! -f .env.development ]; then \
		echo "⚠️  .env.development が見つかりません。テンプレートからコピーします..."; \
		cp .env.development.example .env.development; \
		echo "✅ .env.development を作成しました（必要に応じて編集してください）"; \
	fi
	@echo "📦 Next.jsアプリケーションを作成します..."
	docker compose -f compose.development.yaml --env-file .env.development run --rm nextjsapp \
	bash -c '\
		mkdir -p /tmp/nextjs-init && \
		npx create-next-app@latest /tmp/nextjs-init --typescript --tailwind --eslint --app --src-dir --import-alias "@/*" --use-npm --yes && \
		cp -n /tmp/nextjs-init/package.json /app/package.json && \
		cp -n /tmp/nextjs-init/tsconfig.json /app/tsconfig.json && \
		cp -rn /tmp/nextjs-init/src /app/ && \
		cp -rn /tmp/nextjs-init/public /app/ && \
		rm -rf /tmp/nextjs-init && \
		echo "✅ Next.js ファイルをコピーしました" && \
		npm install prisma @prisma/client next-auth square && \
...
up: ## コンテナを起動
	@if [ ! -f .env.development ]; then \
		...
		echo "  cp .env.development.example .env.development を実行してください。"; \
		exit 1; \
	fi
	docker compose -f compose.development.yaml --env-file .env.development up -d
	@echo "アプリケーションが起動しました: http://localhost:3002"
...
.PHONY: lp-sync
lp-sync: ## projects/ のLP成果物を public/ にコピー（配信用）
	@for dir in projects/*/; do \
		name=$$(basename "$$dir"); \
		mkdir -p "public/lp/$$name"; \
		cp -r "$$dir"* "public/lp/$$name/"; \
		echo "✅ $$name → public/lp/$$name/"; \
	done
