...
```bash
# リポジトリをクローン
git clone https://github.com/AI-LandBase/web.git
cd web
...
- `.env.development.example`: 開発環境用のテンプレート（コミット済み）
- `.env.production.example`: 本番環境用のテンプレート（コミット済み）
- `.env.development`: 実際の開発環境設定（`.gitignore` で除外）
- `.env.production`: 実際の本番環境設定（`.gitignore` で除外）
...
cp .env.development.example .env.development
cp .env.production.example .env.production
# .env.production の値を本番用に編集すること
```

#### 設計方針

- `.example` ファイルをテンプレートとしてコミットし、実体ファイルは `.gitignore` で除外
...
```
.
├── .env.development.example  # 開発環境用環境変数テンプレート
